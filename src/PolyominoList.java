...
public class PolyominoList extends ArrayList<Polyomino>{ //ne g�re pas les exceptions en cas de non-lecture du fichier
	...
	public void draw_aux(int taille, int g, Image2d img){
		for(int i = 0; i < this.size(); i++){
			...
			for(int j = 0; j < a.cases.size(); j++){
				//on r�cup�re tous les carr�s et on les dessine de cette couleur
	...
	public void draw(int taille, int g){ 
		// L'argument taille est l'�cart entre les polyominos
		// g est le facteur de dilatation
		// d�but est la case � partir de laquelle on commence
	...
	public static PolyominoList fixedPolyomino(int area){
		...
		for(int i = 0; i < rec.size(); i++){
			...
			for(int j = 0; j < p.casesVoisines.size(); j++){
				...
				//On enl�ve la case voisine index�e par j
				a.casesVoisines.remove(j);
				
				// On ajoute les nouvelles cases voisines, en �vitant de prendre une case d�j� occup�e
				...
				// on v�rifie que res ne contient pas d�j� le polyomino
				boolean verif = true;
